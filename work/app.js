...
const ects=c=>guideCourse(c.id)?.ects??null;
...
const credited=c=>Number(state.creditTo[c.id]||c.semester);
...
function courseIssues(c){
 ...
 if(c.id==='ECE588'){const passedCredits=state.passed.filter(id=>id!=='ECE588').map(course).filter(Boolean).reduce((n,p)=>n+(ects(p)||0),0);if(passedCredits<180)issues.push(`Διπλωματική: ${passedCredits} καταχωρισμένα περασμένα ECTS έναντι του ελάχιστου 180 (σ. 25).`);issues.push('Η διπλωματική απαιτεί έγκριση του Τμήματος και επιβλέποντα.');}
...
function renderChecks(pairs){
 ...
 if(state.entryYear==='new'&&selected.length){
  ...
  const allocations=Array.from({length:9},(_,i)=>({sem:i+1,items:[...new Set([...state.passed,...chosen()])].map(course).filter(c=>c&&!['ECE121','ECE122'].includes(c.id)&&credited(c)===i+1)}));
 ...
 const specials=[...new Set([...state.passed,...chosen()])].map(course).filter(c=>c&&norm(c.name).includes('ειδικα θεματα'));
